...
public class OneActivity extends Activity {   //OneActivity类头部
   ...
   Timer touchTimer=null;//单击在虚拟按钮上用于连续动作的计时器
   ...
   private LinearLayout linearLayout=null;//Linearlayout线性布局
    ...
    public void OnYS(View v){//创建演示按钮，单击事件方法头部
        if (mdialog==null){//如果自定义对话框为空
            ...
            mdialog=new mCustomProgressDialog(this,"岩石中单击边缘消失",R.drawable.frame1);
        }
        mdialog.show();//显示对话框
          }//创建演示按钮，单击事件方法尾部
            ...
            public boolean onTouch(View v, MotionEvent event) {//
                switch (event.getAction()){  //获取行动方式头部
                    ...
                    case MotionEvent.ACTION_MOVE:  //手势移动中判断
                        ...
                        //下面是根据笔画以及手势做图片的处理，滑动到不同的位置，加载不同的图片
                        if (type==1){
                            //如果当前手指按下的X坐标大于等于图片的X坐标，或者小于等于缩放图片的X坐标时
                            if (x2>=igvx&&x2<=igvx+(int)(arrdown.getWidth()*scaleWidht)){
                                //如果当前手指按下的Y坐标小于等于缩放图片的X坐标，或者大于等于图片的X坐标时
                                if (y2<=igvy+(int)(arrdown.getHeight()*scaleHeight)/24&&y2>=igvy){
                                    lodimagep(1);//调用lodimagep（）方法，加在第一张显示图片
                                }
    ...
    private void jlodimage(){//当手势抬起时数字资源图片倒退显示jlodimage（）方法头部
        if (i==25){//如果当前图片未知等于25
            ...
            //获取图片资源
            int imgid=getResources().getIdentifier(name,"drawable","com.mingrisoft.writenumber3");
            //给imageview设置图片
